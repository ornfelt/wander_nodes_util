...
private:
        ...
        NodeVec const& spawns_a, NodeVec const& spawns_h, NodeVec const& spawns_n, bool immediate, PvPDifficultyEntry const* bracketEntry, NpcBotRegistry* registry)
    {
        ...
        // HEHE: Change wander bot level (setting level here only changes level_nodes added)
        //myminlevel = 80;
        ...
        // HEHE: Change wander bot spawnloc
...
WanderNode const* BotDataMgr::GetNextWanderNode(WanderNode const* curNode, WanderNode const* lastNode, Position const* fromPos, Creature const* bot, uint8 lvl, bool random)
{
    ...
    // HEHE: debug links empty... Shouldn't be
    if (links.empty())
        LOG_ERROR("server.loading", "WAAAARNING links.empty(). BOT ENTRY: {}, lvl: {}, zone: {}, area: {}", bot->GetEntry(), lvl, bot->GetZoneId(), bot->GetAreaId());

    // HEHE: Write position to file
    //std::ofstream outfile;
    //outfile.open("/home/jonas/wander_nodes_test.txt", std::ios_base::app); // append instead of overwrite
    //// No need to close the file manually, as it does so upon destruction
    //outfile << "WanderNodeReached! Bot: " + std::to_string(me->GetEntry()) + ", WPID: " + std::to_string(_travel_node_cur->GetWPId()) + ", pos: " + me->GetPosition().ToString();
