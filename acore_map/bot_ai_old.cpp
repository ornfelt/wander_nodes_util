    ...
    if (IsWanderer())
    {
        if (_baseLevel == 0) //this only happens once
        {
            if (me->GetMap()->IsBattlegroundOrArena())
            {
                ...
                LOG_INFO("server.loading", "BG bot {} id {} selected level {}...", me->GetName().c_str(), me->GetEntry(), uint32(_baseLevel));
            }
            else if (_travel_node_cur != nullptr)
            {
                ...
                mylevel += BotDataMgr::GetLevelBonusForBotRank(me->GetCreatureTemplate()->rank);
                // HEHE: Change wander bot level (if too low, the poor bots will spawn at high-lvl locations unless changed in botdatamgr)
                //if (urand(1, 100) <= 40)
                    //mylevel = 80;
                _baseLevel = std::max<uint8>(mylevel, BotDataMgr::GetMinLevelForBotClass(_botclass));
                // HEHE: wander bot selected level log
        ...
        else if (me->GetMap()->GetEntry()->IsContinent())
        {
            ...
            // HEHE: HEHE: don't level up, but log the "Dings"
            uint8 prevlevel = mylevel;
            mylevel = std::max<uint8>(mylevel, std::min<uint8>(_baseLevel + uint8(_killsCount / (mylevel * 20)), mapmaxlevel));
            if (mylevel != prevlevel)
            {
                LOG_INFO("server.loading", "BOT DING!!! Bot: {}, prevlevel: {}, mylevel: {}, _killsCount: {}", me->GetEntry(), prevlevel, mylevel, _killsCount);
...
uint32 bot_ai::_selectMountSpell() const
{
    ...
    if (!myMountSpellId)
    {
        ...
        // HEHE: Wanderer's can't fly
        //bool can_fly = !IAmFree() ? master->CanFly() : (!instt && me->GetMap()->GetEntry()->addon > 0);
        bool can_fly = !IAmFree() ? master->CanFly() : (!instt && me->GetMap()->GetEntry()->addon > 0) && !IsWanderer();
...
void bot_ai::JustDied(Unit* u)
{
    ...
    // HEHE: don't tele to graveyard if arena
    if (IsWanderer() && me->GetMap()->IsBattlegroundOrArena())
    //if (IsWanderer() && me->GetMap()->IsBattleground())
    {
    ...
    if (dist > 1.5f || IsWanderer())
    {
        ...
        if (!me->isMoving())
        {
            ...
            else if (IsWanderer())
            {
                ...
                outfile.open("/home/jonas/wander_nodes_test.txt", std::ios_base::app); // append instead of overwrite
