...
class DBLayer:
    """Database layer equivalent to PHP DBLayer class"""
    ...
    def __init__(self, host, user, password, database, port=3306):
        ...
        try:
            ...
            print(f"[db] Connected to database: {database}")
    ...
    def query(self, sql):
        if not self.connection:
            print("[db] No valid connection")
            return None
        
        try:
            with self.connection.cursor() as cursor:
                print(f"[db] Executing query: {sql[:100]}...")
                cursor.execute(sql)
                self.num_queries += 1
    ...
    def query_one(self, sql):
        ...
        try:
            with self.connection.cursor() as cursor:
                print(f"[db] Executing single query: {sql[:100]}...")
                cursor.execute(sql)
                self.num_queries += 1
...
def get_zone_name(zone_id):
    """Get zone name by ID"""
    return ZONES.get(zone_id, 'Unknown zone')

def test_realm():
    """Test if realm server is online"""
    try:
        server = CONFIG['server'][CONFIG['realm_id']]['addr']
        port = CONFIG['server'][CONFIG['realm_id']]['game_port']
        print(f"[realm] Testing connection to {server}:{port}")
        ...
        is_online = result == 0
        print(f"[realm] Server status: {'online' if is_online else 'offline'}")
        return is_online
    except Exception as e:
        print(f"[realm] Connection test failed: {e}")
        return False

def sort_players(players):
    """Sort players equivalent to PHP sort_players function"""
    return sorted(players, key=lambda x: (x.get('leaderGuid', 0), x.get('name', '')))
...
def get_players():
    """API endpoint equivalent to pomm_play.php"""
    print("[api] Starting player data fetch")
    
    result = {'status': None, 'online': None}
    
    # Get realm db connection
    realm_db = DBLayer(
    ...
    if not realm_db.is_valid():
        print("[api] Realm database connection failed")
    ...
    characters_db = DBLayer(
        char_db_config['host'],
        char_db_config['user'],
        char_db_config['password'], 
        char_db_config['database'],
        char_db_config['port']
    )
    
    if not characters_db.is_valid():
        print("[api] Characters database connection failed")
        result['status'] = {'online': 2}
        return jsonify(result)
    
    # Constants for race detection
    HORDE_RACES = 0x2B2  # Binary mask for Horde races
    ALLIANCE_RACES = 0x44D  # Binary mask for Alliance races
    OUTLAND_INST = [540,542,543,544,545,546,547,548,550,552,553,554,555,556,557,558,559,562,564,565]
    NORTHREND_INST = [533,574,575,576,578,599,600,601,602,603,604,608,615,616,617,619,624,631,632,649,650,658,668,724]
    
    maps_count = len(LANG_DEFS['maps_names'])
    count = [[0, 0] for _ in range(maps_count)]
    ...
    print("[api] Fetching player data...")
    ...
    for character in merged_results:
        # Determine extension (map type)
        if character['map'] == 530 and character['position_y'] > -1000 or character['map'] in OUTLAND_INST:
            extension = 1
        elif character['map'] == 571 or character['map'] in NORTHREND_INST:
            extension = 2
        else:
            extension = 0
        ...
        if str(character['account']) in gm_accounts:
            ...
            if CONFIG['gm_online']:
                show_player = True
                if (character['extra_flags'] & 0x1) != 0 and CONFIG['map_gm_show_online_only_gmoff']:
                    show_player = False
                if (character['extra_flags'] & 0x10) != 0 and CONFIG['map_gm_show_online_only_gmvisible']:
                    show_player = False
        
        # Count players by faction and extension
        if not gm_player or (gm_player and CONFIG['gm_online_count']):
            if HORDE_RACES & (0x1 << (character['race'] - 1)):
                count[extension][1] += 1
            elif ALLIANCE_RACES & (0x1 << (character['race'] - 1)):
                count[extension][0] += 1
        
        # Count GM online
        if (gm_player and show_player) or (gm_player and not show_player and CONFIG['map_status_gm_include_all']):
            gm_online += 1
        
        if gm_player and not show_player:
            continue
        ...
        char_data = {
            ...
            'name': character['name'],
            'map': character['map'],
            'zone': get_zone_name(character['zone']),
            'cl': character['class'],
            'race': character['race'],
    ...
    if not arr and not test_realm():
        result['online'] = None
    else:
        arr = sort_players(arr)
        # Prepend count data
        final_arr = count + arr
        result['online'] = final_arr
        print(f"[api] Returning {len(arr)} characters with count data")
    
    # Get status information
    if CONFIG['map_show_status']:
        #uptime_query = """
        #    SELECT UNIX_TIMESTAMP() as current_time, `starttime`, `maxplayers` 
        #    FROM `uptime` 
        ...
        uptime_query = """
            ...
            WHERE `starttime`=(SELECT MAX(`starttime`) FROM `uptime`)
        """
        uptime_result = realm_db.query_one(uptime_query)
        
        if uptime_result:
            #status = {
            #    'online': 1 if test_realm() else 0,
            #    'uptime': uptime_result['current_time'] - uptime_result['starttime'],
            #    'maxplayers': uptime_result['maxplayers'],
            #    'gmonline': gm_online
            #}
            status = {
                'online': 1 if test_realm() else 0,
                'uptime': current_timestamp - uptime_result['starttime'],
                'maxplayers': uptime_result['maxplayers'],
                'gmonline': gm_online
            }
...
def get_realm_name():
    ...
    if realm_db.is_valid():
        query = f"SELECT name FROM realmlist WHERE id = {CONFIG['realm_id']}"
        result = realm_db.query_one(query)
        realm_db.close()
        if result:
            return result['name']
